...
def __divide_dataset():
    ...
    with open(path, 'r') as reader:
        for line in reader.readlines():
            ...
            v = v[np.newaxis, :]  # 插入新维度
            if k in data.keys():
                data[k] = np.concatenate((data[k], v), axis=0)  # 按轴axis连接array组成一个新的array
            else:
                data[k] = v
    ...
    # 存储数据集
    with h5py.File('Data/heart_classification.h5', 'w') as hf:
        cnt = 0
...
def __statistic_dataset():
    ...
    # 获取特征最小值、最大值
    with h5py.File('Data/heart_classification_train.h5', 'r') as hf:
...
def __divide_train_val_dataset(ratio = 0.2, fold = 0):
    ...
    # 按比例划分训练集和验证集
    with h5py.File('Data/heart_classification.h5', 'r') as reader:
        # 保存验证集
        with h5py.File('Data/heart_classification_val.h5', 'w') as writer:
            for i in range(category):
                # 读取数据
                ...
                stride = int(len / 3)
                # 保存验证集
                writer[str(i)] = data[stride * 1 : stride * 2]

        # 保存训练集
        with h5py.File('Data/heart_classification_train.h5', 'w') as writer:
            for i in range(category):
                # 读取数据
                data = np.asarray(reader[str(i)])
                # 数据集大小
                ...
                # 保存训练集
                writer[str(i)] = np.concatenate((data[stride * 0 : stride * 1], data[stride * 2 : ])
                                        ,axis=0)
    print("划分数据集已完成")
...
def __generate_sub_dataset():
    ...
    # 设置子集每个类别数量
    threshold = round(math.sqrt(min_sample_amount * max_sample_amount))
    # 设置子集数量
    ...
    for i in range(0, end_index):
        ...
        if amo < threshold and amo > k_neighbors * 3:  # 使用SMOTE算法增强数据，增强数量最多为原来的一半
            ...
            x, y = data[:, :-1], data[:, -1]
            x, y = SMOTE(random_state=1, k_neighbors=k_neighbors).fit_resample(x, y)
            y = y[np.newaxis, :]
            data = np.c_[x, y.T]
            # 少类增强后的结果
    ...
    # 基于滑动窗口对多类采样
    with h5py.File('Data/heart_classification_subset_train.h5', 'w') as hf:
        for i in range(num):
...
if __name__ == '__main__':
    ...
    # 划分训练集和验证集
    __divide_train_val_dataset(ratio=0.33, fold=2)
