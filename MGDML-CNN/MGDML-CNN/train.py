...
def train_triplet_module():
    ...
    margin = 1e-3 # triplet loss 超参数 margin
    interval = 5  # diff loss和triplet
    balance = 0  # diff loss和triplet loss间权重
    ...
    for epoch in range(start_epoch, EPOCH):
        ...
        for data in train_loader:
            ...
            triplet_loss = batch_hard_triplet_loss(num_subset_sample, labels, embeddings, margin=margin, device=device)
            # triplet loss
...
def train_knn_module():
    ...
    x_trains, y_train = [np.zeros((0, 8)) for i in range(NUM_MODEL)], np.zeros((0, ))
    # 获取测试数据embedding、labels
    x_tests, y_test = [np.zeros((0, 8)) for i in range(NUM_MODEL)], np.zeros((0,))
    ...
    # 计算f1
    f1 = f1_score(y_test, y_pr, average='micro')
    # 计算G-mean
    g_mean = geometric_mean_score(y_test, y_pr, average='micro')
    # 计算auc
    auc = roc_auc_score(label_binarize(y_test, np.arange(N_CLASS)), y_pr_p, average='micro')
